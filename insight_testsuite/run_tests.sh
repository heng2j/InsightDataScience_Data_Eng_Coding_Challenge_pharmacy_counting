...
function setup_testing_input_output {
  ...
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/de_cc_data_head500.txt ${TEST_OUTPUT_PATH}/input/de_cc_data_head500.txt
}

# Keep the original compar_outputs
...
function compare_final_outputs {
  NUM_OUTPUT_FILES_PASSED=0

	DICTIONRY_ANSWER_PATH=${GRADER_ROOT}/output/top_cost_drug_large.txt
	MAPREDUCE_ANSWER_PATH=${GRADER_ROOT}/output/top_cost_drug_large_mapreduce.txt

	DIFF_RESULT1=$(diff -bB ${DICTIONRY_ANSWER_PATH} ${MAPREDUCE_ANSWER_PATH} | wc -l)
	if [ "${DIFF_RESULT1}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH1} ]; then
	echo "Compare Final outputs from Dictionary approach and Mapreduce approach: "
	echo -e "[${color_green}PASS${color_norm}]: ${DICTIONRY_ANSWER_PATH} ${MAPREDUCE_ANSWER_PATH}"
	NUM_OUTPUT_FILES_PASSED=$(($NUM_OUTPUT_FILES_PASSED+1))
	else
	echo -e "[${color_red}FAIL${color_norm}]: ${test_folder}"
	diff ${DICTIONRY_ANSWER_PATH} ${MAPREDUCE_ANSWER_PATH}
	fi
...
function run_all_tests {
  ...
  for test_folder in ${TEST_FOLDERS}; do
    ...
    cd ${GRADER_ROOT}/temp
    bash run_test.sh 2>&1
    cd ../

    output_files=("top_cost_drug.txt" "top_cost_drug_head500.txt")
    compare_outputs "${output_files[@]}"
  done
